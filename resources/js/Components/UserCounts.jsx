...
const UserCounts = (props) => {
    const [date, setDate] = useState('December 2023')
    const [highestUser, sethighestUser] = useState(3)
    const handleDateChange = (event) => {
        setDate(event.target.value);
    };

    const totaluser = props.totaluser.filter(item => item.dates === date);

    const totalusers = totaluser.reduce((sum, currentTotal) => {
        return sum + currentTotal.total;
    }, 0);

    const handleStateUpdate = () => {
        const highestTotalItem = totaluser.reduce((maxItem, currentItem) => {
            return currentItem.total > maxItem.total ? currentItem : maxItem;
        }, totaluser[0]);
        sethighestUser(highestTotalItem.total);
    };
    ...
    const yearMonths = Array.from(new Set(props.totaluser.map(item => item.dates)));
    return (
        <>
            <div className="w-full bg-white mt-[2vw] pt-[2vw] pb-[5vw] px-[3vw] relative">
                <div className="w-full flex justify-between mb-[3vw]">
                    <div>
                        ...
                        <select name="date" id="date" className="outline-none text-[0.9vw] mt-[1vw]" value={date} onChange={handleDateChange}>
                ...
                <LineUserCounts totaluser={props.totaluser} />
            </div>
        </>
    )
}
